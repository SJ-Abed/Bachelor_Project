...
orders_df = orders_df.groupby(['bid', 'cid', 'checkoutdate', 'classid', 'catid', 'segmentationlabel', 'days',
                               'marketid']).sum().reset_index().sort_values('checkoutdate', ascending=False)
...
########################################################################################
#remove outlier purchases (orders with more than 60 days interval with next one)
...
before_gap_orders = df[df.days_untill_next > 60]
...
#####important
#remove first order of each customer if the interval between first and second order is more than 60 days
...
#####important
#remove first two orders of each customer if the interval between first and second, and second and third order is more than 60 days
...
# Save Prepared Data
train_validation_merge = train_validation_merge[train_validation_merge['days_since_prior_order']<=60]
test_validation_merge = test_validation_merge[test_validation_merge['days_since_prior_order']<=60]
train_validation_merge.to_csv('train_validation_merge_v2_no_outlier.2.csv')
test_validation_merge.to_csv('test_validation_merge_v2_no_outlier.2.csv')
